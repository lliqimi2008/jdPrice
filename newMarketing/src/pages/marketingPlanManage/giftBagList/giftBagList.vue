<template>
  <div class="gift-bag-list">
    <div class="pub-sm-title clear">
      <h3 class="fl">礼包列表</h3>
      <el-row class="fr">
        <el-button size="mini" type="primary" @click="showGiftBagPop('create')" v-hasPermissions="'marketingPlanManage:planDetail:addGiftBag'">新增礼包</el-button>
      </el-row>
    </div>
        ...
        <div class="gift-bag-body">
          <div class="list" v-for="(item, index) in giftBagList" :key="index">
            ...
            <div class="operation text-center">
              <el-button type="text" size="small" @click="showGiftBagPop({item: item, bagPrizeList: item.bagPrizeList})" v-hasPermissions="'marketingPlanManage:planDetail:editGiftBag'">编辑</el-button>
              <el-button type="text" size="small" @click="deleteGiftBag(item)" v-hasPermissions="'marketingPlanManage:planDetail:delGiftBag'">删除</el-button>
            </div>
          </div>
        </div>
      </div>
      ...
      <el-dialog title="新增礼包" :visible.sync="giftBag">
        <el-form class="app-dialog-form" label-position="left" label-width="90px" size="small" ref="giftBag" :model="giftBagForm" :rules="giftBagFormRules">
          ...
          <el-form-item size="large">
            <el-button type="primary" :disabled="butDisabled" @click="createGiftBagValidate('giftBag')">保存</el-button>
  ...
  methods: {
    ...
    handleSelectionChange(data) {
      ...
      data.forEach((item, index) => {
        ...
        // this.editCurMemrych.push(item.prizeId)
      })
      this.prizeIds = tempArr.ids
      this.prizeTotals = tempArr.totals
      // this.editCurAll = this.removeDuplicatedItem(this.editCurMemrych)
      /* if (tempArr.ids.length > 0) {
          let legOne = this.editCurAll[this.editCurAll.length - 1]
          let legTwo = tempArr.ids[tempArr.ids.length - 1]
          if (legOne !== legTwo) {
            this.editCurAll.pop()
          }
        } */
