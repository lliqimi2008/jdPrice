<template>
  <el-form class="app-form-panel"
    ...
    ref="ruleForm" :model="ruleForm" :rules="ruleFormRules">
      ...
      <el-form-item label="用户状态：" prop="calValue">
        <el-radio-group v-model="ruleForm.calValue">
          <el-radio label="Y">付款码新用户</el-radio>
          <el-radio label="N">付款码老用户</el-radio>
        </el-radio-group>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      butDisabled: false,
      name: 'isPaymentCodeNewUser',
      ruleCodeName: 'rule38',
      ruleForm: {
        ruleName: '付款码新老用户限制',
        instructions: '选择可参加活动的付款码用户状态',
        noPassDesc: '非活动指定的付款码用户'
      },
      // 保存规则数据
      requestRelesData: {
        // 规则类型
