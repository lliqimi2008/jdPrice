...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        calType: '>=',
        ruleName: '商城订单品类/SKU限制',
        instructions: '对参与活动的SKU或品类进行限制',
  ...
  methods: {
    editDefaultVal(data) {
      ...
      // console.log(JSON.parse(extension))
      let keys = extension.replace(/[\\{"}]/g, '').split(':')
      this.ruleForm.limitType = keys[0] || 'skuIdAllIn'
      this.ruleForm.sku = keys[1] || ''

      // 如果有多个扩展值组合，通过for in 取key value
      // var objs = extension // {"skuIdAllNotIn":"2"}
      /* for(var key in objs) {
        // 这种方式需要对应传输的key与声明的key相同
        this.ruleForm[key] = objs[key]
