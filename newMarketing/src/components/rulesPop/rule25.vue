<template>
  <el-form class="app-form-panel"
    ...
    ref="ruleForm" :model="ruleForm" :rules="ruleFormRules">
      ...
      <el-form-item label="银行卡编码:" prop="calValue">
        <el-input v-model="ruleForm.calValue" placeholder="支持多个，与'|'分隔 " size="small"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        ruleName: '银行卡编码',
        instructions: '限制使用的银行卡行',
        noPassDesc: '非活动指定银行卡编码'
      },
      // 保存规则数据
      requestRelesData: {
        // 规则类型
      ...
      ruleFormRules: {
        calValue: [
          {
            required: true,
            message: '请输入可参加活动银行的卡编码，如扫行为：CBM',
            trigger: 'change'
          }
        ],
        noPassDesc: [
    ...
    submitRule(formName) {
      this.$refs[formName] &&
        this.$refs[formName].validate(valid => {
          ...
          // 设置公共属性值
          this.setAttribute()
          this.requestRelesData.calValue = this.replaceSeparator(this.ruleForm.calValue)
