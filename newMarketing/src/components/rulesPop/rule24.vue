<template>
  <el-form class="app-form-panel"
    ...
    ref="ruleForm" :model="ruleForm" :rules="ruleFormRules">
      ...
      <el-form-item label="支付产品：" prop="types">
        <el-checkbox-group v-model="ruleForm.types">
          <el-checkbox label="JDPAY02" name="types">在线支付</el-checkbox>
          <el-checkbox label="JDPAY02SYS" name="types">自营扫一扫</el-checkbox>
          <el-checkbox label="JDPAY02FKM" name="types">自营付款码</el-checkbox>
          <el-checkbox label="JDPAY02QP" name="types">京东闪付</el-checkbox>
          <el-checkbox label="UNPQRSCAN" name="types">银联扫一扫</el-checkbox>
          <el-checkbox label="UNPQRPAY" name="types">银联付款码</el-checkbox>
          <el-checkbox label="AGENTREC" name="types">代扣</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        ruleName: '支付产品限制',
        instructions: '限制使用的京东支付产品',
        types: [],
        noPassDesc: '非活动指定支付产品'
      },
      // 保存规则数据
      requestRelesData: {
        // 规则类型
  ...
  methods: {
    editDefaultVal(data) {
      this.ruleForm.types = data.calValue.split('|')
    },
    submitRule(formName) {
      this.$refs[formName] &&
        this.$refs[formName].validate(valid => {
          ...
          this.setAttribute()
          // 设置计算值（多个计算值使用|分隔）
          this.requestRelesData.calValue = this.ruleForm.types.join('|')
