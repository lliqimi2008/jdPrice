<template>
  <el-form class="app-form-panel"
    ...
    ref="ruleForm" :model="ruleForm" :rules="ruleFormRules">
      ...
      <el-form-item label="外部商户SKU值：" prop="sku">
        <el-input
          type="textarea"
          :rows="5"
          placeholder="请输入内容"
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        calType: '>=',
        ruleName: '外部商户订单SKU限制',
        instructions: '外部自营商户订单参与活动的SKU限制',
        noPassDesc: '订单中含有不可参加活动sku'
      },
      // 保存规则数据
      requestRelesData: {
        // 规则类型
  ...
  methods: {
    editDefaultVal(data) {
      ...
      this.ruleForm.sku = extension.skuIdAllIn || ''
    },
    submitRule(formName) {
      this.$refs[formName] &&
