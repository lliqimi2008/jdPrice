<template>
  <el-form class="app-form-panel"
    ...
    <el-row>
      <el-col :span="13">
        <el-form-item label="金额限制：" prop="calType">
          <el-select class="select100" v-model="ruleForm.calType" placeholder="请选择" size="small">
            <el-option
  ...
  data() {
    return {
      ...
      ruleForm: {
        calType: '>=',
        ruleName: '订单金额限制',
        instructions: '订单参与活动的金额门槛',
        noPassDesc: '订单金额不满足规则要求'
      },
      // 保存规则数据(默认值 ，可被替换)
      requestRelesData: {
        // 规则类型
