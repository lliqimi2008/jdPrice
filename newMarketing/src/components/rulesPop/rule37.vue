  ...
  <el-form class="app-form-panel"
    ...
    ref="ruleForm" :model="ruleForm" :rules="ruleFormRules">
      ...
      <el-form-item label="会员类型：" prop="vipType">
        <el-checkbox-group v-model="ruleForm.vipType">
          <el-checkbox label="a">新户</el-checkbox>
          <el-checkbox label="b">首单用户</el-checkbox>
          <el-checkbox label="c">月结用户</el-checkbox>
          <el-checkbox label="d">周结用户</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        ruleName: '金采用户分类',
        instructions: '选择可参加活动的金采用户类型',
        vipType: [],
        noPassDesc: '非指定可参与活动的金采用户'
      },
      // 保存规则数据
      requestRelesData: {
        // 规则类型
  ...
  methods: {
    editDefaultVal(data) {
      this.ruleForm.vipType = data.calValue.split('|')
    },
    submitRule(formName) {
      this.$refs[formName] &&
        this.$refs[formName].validate(valid => {
          ...
          this.setAttribute()
          // 设置计算值（多个计算值使用|分隔）
          this.requestRelesData.calValue = this.ruleForm.vipType.join('|')
