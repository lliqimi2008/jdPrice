  ...
  <div class="width100">
    <el-form class="app-dialog-form demo-form-inline"
      ...
      v-if="showRuleType === 'group'">
        <el-form-item label="奖项规则:">
          <el-col :span="4">
            <el-button size="mini" type="primary" @click="addRuleGroup" v-hasPermissions="'marketingPlanManage:createAward:addNewRules'">添加新的规则组</el-button>
          </el-col>
          <el-col :span="9">
...
export default {
  ...
  methods: {
          ...
          this.$confirm('确认删除?', '提示', {
            ...
            .then(() => {
              ...
              deleteRule(reqParams)
                .then(res => {
                  if (!this.httpThen(this, res)) return
                  if (this.rulesGroupData.length > 1) {
    ...
    addRuleGroup() {
      // 添加规则组标识
      store.dispatch('setIsAddRuleGroup', true)
    ...
    delRuleGroupConfirm() {
      this.$confirm('确认删除“规则组' + this.groupName + '”吗?', '提示', {
        ...
        .then(() => {
          // 删除规则组
          this.delRuleGroup(this.groupName)
        })
        .catch(() => {})
    },
    // 删除规则组
    delRuleGroup(e) {
      ...
      try {
        let reqParams = {
          ...
          // 规则组
          calGroupid: e || this.rulesGroupData[0].calGroupId
        }
