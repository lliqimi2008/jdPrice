<template>
  <el-form class="app-form-panel"
    ...
    ref="ruleForm" :model="ruleForm" :rules="ruleFormRules">
      ...
      <el-form-item label="京东支付用户类型:" prop="calValue">
        <el-radio-group v-model="ruleForm.calValue">
          <el-radio label="1" name="calValue">新用户</el-radio>
          <el-radio label="0" name="calValue">老用户</el-radio>
        </el-radio-group>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      butDisabled: false,
      name: 'jdPayCustomerPotential',
      ruleCodeName: 'rule30',
      ruleForm: {
        ruleName: '京东支付新老用户',
        instructions: '限制参加活动的京东支付用户',
        noPassDesc: '非活动指定京东支付用户'
      },
      // 保存规则数据
      requestRelesData: {
        // 规则类型
