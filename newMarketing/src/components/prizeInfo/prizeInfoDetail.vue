<template>
  <div class="prize-info-detail">
    ...
    <div class="pv prize-content">
      <el-button v-if="awardsData.length" class="pa edit-btn" size="mini" type="primary" @click="activityIsAllowEdit" v-hasPermissions="'marketingPlanManage:createAward:editorAward'">编辑奖项</el-button>
      <not-data v-if="!awardsData.length" text="暂无奖项信息">
        <app-loading slot="loading" :height="120" v-if="awardsLoading"></app-loading>
        <div slot="btns" class="mar-t-10"><el-button v-if="!awardsData.length" size="mini" type="primary" @click="createAwards" v-hasPermissions="'marketingPlanManage:createAward:createAward'">立即创建奖项</el-button></div>
      </not-data>
      <el-tabs v-else
...
import { queryAwardsGetList, activityIsAllowEdit } from 'api/service'
